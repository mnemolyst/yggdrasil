...
for i in range(rings):
    ...
    for j in range(el_per_ring):
        ...
        if abs(maxdp) > .99:
            ...
            #elements['2x4'][len(orientations)-1] = []
            #elements['2x4'][len(orientations)-1].append(el)
            elements.append({
                'nodes': el,
                'section': '.292,.125', # two by four, 3.5" x 1.5"
                'material': 'WOOD',
...
fout.write('1,' + str(sum([len(x) for x in elements])) + '\n')
...
for i, es in enumerate(elsets):
    fout.write('*BEAM SECTION,ELSET=ES' + str(i + 1) + '\n')
